...
async def get_user(conn: Connection, *, token: str = None, name: str = None) -> User:
  user_record = await conn.fetchrow("SELECT * FROM Users WHERE Token = $1 OR Username = $2;", token, name)
...
async def authenticate(request: Request) -> Key|User|Response:
  ...
  if auth_token is None:
    return Response(status=401, body="pass Authorization header or Authorization cookie.")
  ...
  if len(auth_token) != config.SECURITY.TOKEN_LENGTH:
    return Response(status=400,body=f"auth token length invalid. expected {config.SECURITY.TOKEN_LENGTH} but got {len(auth_token)}.")
  if not auth_token.isalnum():
    return Response(status=400,body="auth token invalid. must be alphanumeric.")
  ...
  async with pool.acquire() as conn:
    ...
    except: pass # If it can't find it, no biggie here, but it will throw a
    ...
    user_record = await conn.fetchrow("SELECT * FROM Users WHERE Token=$1 OR Password=$1;", auth_token)
    user = None
    try:
      user = User.from_record(user_record)
    except: pass # If it can't find it, no biggie here also.
...
async def user_authenticate_secure(request: Request) -> Response|User:
  ...
  async with pool.acquire() as conn:
    ...
    user_record = await conn.fetchrow("SELECT * FROM Users WHERE Password = $1;", auth_token)
    try:
      user = await User.from_record(user_record)
    except: pass # If it can't find it, no bigger here also.
                 # We can just return a 401 response.
    if user:
      return user # We found a user, return it.
    # Last step, if all else fails, return a Response with a 401.
